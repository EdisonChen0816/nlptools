...
class Month(Datetime):
    ...
    def __init__(self, info):
        ...
        if info[1] == "Month":
            ...
            self.datetime = self.datetime + relativedelta(months=int(info[2]))
        else:
            raise("wrong paramater")
    ...
    def __init__(self, info):
        ...
        if info[1] == "Day":
            ...
            self.datetime = self.datetime + datetime.timedelta(
                days=int(info[2]) - datetime.datetime.now().weekday() - 1)
        else:
            raise("wrong paramater")
...
class Weekday(Datetime):
    ...
    def __init__(self, info):
        Datetime.__init__(self, info)
        if info[1] == "Weekday":
            self.datetime = self.datetime.replace(hour=int(info[2]))
        elif (info[1] == "RelWeek"):
            self.datetime = self.datetime + datetime.timedelta(days=int(info[2]) * 7)
        else:
            raise("wrong paramater")
    ...
    def reduce(self, t):
        if isinstance(t, Hour):
            t.datetime = t.datetime.replace(
                year=self.datetime.year, month=self.datetime.month,
                day=self.datetime.day, hour=t.datetime.hour + self.datetime.hour)
            t.term = self.term + "," + t.term
            t.weight = max(self.weight, t.weight)
            return t
        else:
...
class Hour(Datetime):
    ...
    def reduce(self, t):
        if isinstance(t, Minute):
            t.datetime = t.datetime.replace(
                year=self.datetime.year, month=self.datetime.month,
                day=self.datetime.day, hour=self.datetime.hour, minute=t.datetime.minute)
            t.term = self.term + "," + t.term
            t.weight = max(self.weight, t.weight)
            return t
        else:
...
class Minute(Datetime):
    ...
    def __init__(self, info):
        Datetime.__init__(self, info)
        if info[1] == "Minute":
            self.datetime = self.datetime.replace(minute=int(info[2]))
        else:
            raise "wrong paramater"
