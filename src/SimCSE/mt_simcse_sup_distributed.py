...
def eval(model, dataloader) -> float:
    ...
    with torch.no_grad():
        for source, target, label in dataloader:
            ...
            source_attention_mask = source['attention_mask'].squeeze(1).cuda()
            source_token_type_ids = source['token_type_ids'].squeeze(1).cuda()
            source_pred = model(source_input_ids, source_attention_mask, source_token_type_ids)
            # target        [batch, 1, seq_len] -> [batch, seq_len]
            target_input_ids = target['input_ids'].squeeze(1).to(DEVICE)
            target_attention_mask = target['attention_mask'].squeeze(1).cuda()
            target_token_type_ids = target['token_type_ids'].squeeze(1).cuda()
...
def train(model, train_dl, optimizer, DEVICE) -> None:
    """模型训练函数
    """
    model.train()
    global best
    ...
    for batch_idx, source in enumerate(tqdm(train_dl), start=1):
        ...
        input_ids = source.get('input_ids').view(real_batch_num * 3, -1).cuda(non_blocking=True)
        attention_mask = source.get('attention_mask').view(real_batch_num * 3, -1).cuda(non_blocking=True)
        token_type_ids = source.get('token_type_ids').view(real_batch_num * 3, -1).cuda(non_blocking=True)
        # 训练
...
def main_worker(loacl_rank, nprocs):
    dist.init_process_group(backend='nccl', init_method='tcp://127.0.0.1:51789', rank=loacl_rank, world_size=nprocs)
    torch.cuda.set_device(loacl_rank)
    DEVICE = torch.device('cuda:' + str(loacl_rank) if torch.cuda.is_available() else 'cpu')
    ...
    model = SimcseModel(pretrained_model=model_path, pooling=POOLING).cuda()
    model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[loacl_rank], output_device=[loacl_rank], find_unused_parameters=True)
